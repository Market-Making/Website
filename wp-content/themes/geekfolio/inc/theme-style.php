<?php
function geekfolio_theme_styles()  
{ 
	...
	wp_enqueue_style('geekfolio-style', get_stylesheet_directory_uri() . '/style.css', array(), '1.3.0', 'all' );
	wp_style_add_data('geekfolio-style', 'rtl', 'replace' );
	if( is_rtl() ) {
		wp_enqueue_style('geekfolio-theme-rtl', get_stylesheet_directory_uri() . '/assets/css/theme-rtl.css', array(), '1.3.0', 'all' );
	}
...
function geekfolio_fonts_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Google font: on or off', 'geekfolio' ) ) {
...
*/
function geekfolio_fonts_style() {
	wp_enqueue_style( 'geekfolio-fonts', geekfolio_fonts_url(), array(), '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'geekfolio_fonts_style' );

//for google font  in editor
function geekfolio_fonts_editor_style() {
	$font_url = add_query_arg( 'family', urlencode( 'Open Sans:300,400,400i,500,600,600i,700,700i,800,800i,900|Poppins:300,400,500,600,700,800,900|Barlow Condensed:300,400,500,600,700,800,900' ), "//fonts.googleapis.com/css" );
	add_editor_style( $font_url );
}
add_action( 'after_setup_theme', 'geekfolio_fonts_editor_style' );

/*
Enqueue admin scripts and styles.
*/
function geekfolio_admin_style() {
	wp_enqueue_style('geekfolio-admin-custom', get_stylesheet_directory_uri() . '/assets/css/admin-custom.css', array(), '5.1.1', 'all' );
}
add_action( 'admin_enqueue_scripts', 'geekfolio_admin_style' );


//Register and load FontAwesome to WP Editor

add_action( 'enqueue_block_editor_assets', 'geekfolio_editor_fontawesome' );
function geekfolio_editor_fontawesome() {

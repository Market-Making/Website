jQuery(document).ready( function( $ ) {
	...
	$( document ).on( 'click', '.geekfolio_tab_reset_settings', function() {

		var reset_message = geekfolio_admin_meta.reset_message;
		...
		if( flag ) {
			...
			var reset_tab = $( this ).parents( '.geekfolio_meta_box_tab' );
	        ...
	        reset_tab.find( '.geekfolio_remove_button, .multiple_images .remove' ).trigger( 'click' );
	    }
	});

	// Check on load for selected tab when user come before if not it show first one active
	if($.cookie('geekfolio_metabox_active_id_' + $('#post_ID').val())) {
		var active_class = $.cookie('geekfolio_metabox_active_id_' + $('#post_ID').val());

		$('#geekfolio_admin_options').find('.geekfolio_meta_box_tabs li').removeClass('active');
		$('#geekfolio_admin_options').find('.geekfolio_meta_box_tab').removeClass('active').hide();

		$('.'+active_class).addClass('active').fadeIn();
		$('#geekfolio_admin_options').find('#'+active_class).addClass('active').fadeIn();

	} else {
		$('.geekfolio_meta_box_tabs li:first-child').addClass('active');
		$('.geekfolio_meta_box_tab_content .geekfolio_meta_box_tab:first-child').addClass('active').fadeIn();
	}
	$('.geekfolio_meta_box_tabs li a').click(function(e) {
		e.preventDefault();

		var tab_click_id = $(this).parent().attr('class').split(' ')[0];
		var tab_main_div = $(this).parents('#geekfolio_admin_options');

		$.cookie('geekfolio_metabox_active_id_' + $('#post_ID').val(), tab_click_id, { expires: 7 });
		
		tab_main_div.find('.geekfolio_meta_box_tabs li').removeClass('active');
		tab_main_div.find('.geekfolio_meta_box_tab').removeClass('active').hide();
  ...
  /* Metabox dependance of fields */
    ...
    $(".geekfolio_select_parent").change(function () {
	    var str_selected = $(this).find("option:selected").val();
	    var tab_active_status_main = $(this).parents('#geekfolio_admin_options');
		...
		if(str_selected == 'geekfolio_layout_both_sidebar'){
			$('.geekfolio_layout_left_sidebar_single').removeClass('hide_dependency');
			$('.geekfolio_layout_left_sidebar_single').find('input[type="hidden"]').val('1');
			$('.geekfolio_layout_right_sidebar_single').removeClass('hide_dependency');
			$('.geekfolio_layout_right_sidebar_single').find('input[type="hidden"]').val('1');
		}
    ...
    if( $( 'body' ).hasClass('post-type-portfolio') ) {
		...
		$( '#geekfolio_portfolio_post_type_single' ).val( defaultPortfolioSelect );
		$( '.post-type-portfolio').on( 'click', '.post-format', function() {
			var clickVal = $(this).val();
			if( clickVal == 'link' ){
				$( '#geekfolio_portfolio_post_type_single' ).val( clickVal );
			} else {
				$( '#geekfolio_portfolio_post_type_single' ).val( 'standard' );
			}
		});
	}

    var link_color = $( '.geekfolio-color-picker' );
    ...
    $( document ).on( 'click', '.geekfolio_upload_button', function(event) {
          var file_frame;
        var button = $(this);
    ...
    $( document ).on( 'click', '.geekfolio_remove_button', function(event) {
    ...
    /* multiple image upload */
      ...
      $( document ).on( 'click', '.geekfolio_upload_button_multiple', function(event) {
            var file_frame;
          var button = $(this);
	...
	function post_format_selection_options() {
			...
			if ($('#post-format-link').is(':checked')) {
				$('.geekfolio_portfolio_external_link_single_box').fadeIn();
			}else{
				$('.geekfolio_portfolio_external_link_single_box').hide();
			}
			
			$('body.post-type-post #geekfolio_admin_options_single').hide();

	       if ($('#post-format-gallery').is(':checked')) {
	        	$('body.post-type-post #geekfolio_admin_options_single').show();
	            $('.geekfolio_gallery_single_box').fadeIn();
	            $('.geekfolio_lightbox_image_single_box').fadeIn();
	            $('.geekfolio_quote_single_box').hide();
	            $('.geekfolio_link_type_single_box').hide();
	            $('.geekfolio_link_single_box').hide();
	            $('.geekfolio_video_mp4_single_box').hide();
	            $('.geekfolio_video_ogg_single_box').hide();
	            $('.geekfolio_video_webm_single_box').hide();
	            $('.geekfolio_video_single_box').hide();
	            $('.geekfolio_video_type_single_box').hide();
	            $('.geekfolio_audio_single_box').hide();
	            $('.geekfolio_enable_mute_single_box').hide();
	            $('.geekfolio_featured_image_single_box').fadeIn();
	            $('.geekfolio_subtitle_single_box').fadeIn();
	            $(".geekfolio_enable_mute_single_box").removeClass('show_div').removeClass('hide_div');
				$(".geekfolio_video_mp4_single_box").removeClass('show_div').removeClass('hide_div');
				$(".geekfolio_video_ogg_single_box").removeClass('show_div').removeClass('hide_div');
				$(".geekfolio_video_webm_single_box").removeClass('show_div').removeClass('hide_div');
				$(".geekfolio_video_single_box").removeClass('show_div').removeClass('hide_div');

	        } else if ($('#post-format-video').is(':checked')) {
	        	$('body.post-type-post #geekfolio_admin_options_single').show();
	            $('.geekfolio_gallery_single_box').hide();
	            $('.geekfolio_lightbox_image_single_box').hide();
	            $('.geekfolio_quote_single_box').hide();
	            $('.geekfolio_link_type_single_box').hide();
	            $('.geekfolio_link_single_box').hide();
	            $('.geekfolio_video_mp4_single_box').fadeIn();
	            $('.geekfolio_video_ogg_single_box').fadeIn();
	            $('.geekfolio_video_webm_single_box').fadeIn();
	            $('.geekfolio_video_single_box').fadeIn();
	            $('.geekfolio_video_type_single_box').fadeIn();
	            $('.geekfolio_audio_single_box').hide();
	            $('.geekfolio_enable_mute_single_box').fadeIn();
	            $('.geekfolio_featured_image_single_box').fadeIn();
	            $('.geekfolio_subtitle_single_box').fadeIn();
	            ...
	            $('.geekfolio_video_single_box').hide();
	            $('.geekfolio_video_type_single_box').hide();
	            $('.geekfolio_audio_single_box').fadeIn();
	            $('.geekfolio_enable_mute_single_box').hide();
	            $('.geekfolio_featured_image_single_box').fadeIn();
	            $('.geekfolio_subtitle_single_box').fadeIn();
	            $(".geekfolio_enable_mute_single_box").removeClass('show_div').removeClass('hide_div');
	            ...
	            $('.geekfolio_gallery_single_box').hide();
	            $('.geekfolio_lightbox_image_single_box').hide();
	            $('.geekfolio_quote_single_box').fadeIn();
	            $('.geekfolio_link_type_single_box').hide();
	            $('.geekfolio_link_single_box').hide();
	            $('.geekfolio_video_mp4_single_box').hide();
	            $('.geekfolio_video_ogg_single_box').hide();
	            ...
	            $('.geekfolio_lightbox_image_single_box').hide();
	            $('.geekfolio_quote_single_box').hide();
	            $('.geekfolio_image_single_box').fadeIn();
	            $('.geekfolio_link_type_single_box').hide();
	            $('.geekfolio_link_single_box').hide();
	            $('.geekfolio_video_mp4_single_box').hide();
	            $('.geekfolio_video_ogg_single_box').hide();
	...
	$('#geekfolio_video_type_single').change(function() {
		post_format_video_selection();
	});

	function post_format_video_selection(){
		if( $('#geekfolio_video_type_single').val() == 'self' && $('#post-format-video').is(':checked') ){
			$(".geekfolio_enable_mute_single_box").addClass('show_div').removeClass('hide_div');
			$(".geekfolio_video_mp4_single_box").addClass('show_div').removeClass('hide_div');
			$(".geekfolio_video_ogg_single_box").addClass('show_div').removeClass('hide_div');
			$(".geekfolio_video_webm_single_box").addClass('show_div').removeClass('hide_div');
			$(".geekfolio_video_single_box").removeClass('show_div').addClass('hide_div');
		} else if( $('#geekfolio_video_type_single').val() == 'external' && $('#post-format-video').is(':checked') ){
			$(".geekfolio_enable_mute_single_box").removeClass('show_div').addClass('hide_div');
			$(".geekfolio_video_mp4_single_box").removeClass('show_div').addClass('hide_div');
			$(".geekfolio_video_ogg_single_box").removeClass('show_div').addClass('hide_div');
			$(".geekfolio_video_webm_single_box").removeClass('show_div').addClass('hide_div');
			$(".geekfolio_video_single_box").addClass('show_div').removeClass('hide_div');
		} else{
			$(".geekfolio_enable_mute_single_box").removeClass('show_div').removeClass('hide_div');
			$(".geekfolio_video_mp4_single_box").removeClass('show_div').removeClass('hide_div');
			$(".geekfolio_video_ogg_single_box").removeClass('show_div').removeClass('hide_div');

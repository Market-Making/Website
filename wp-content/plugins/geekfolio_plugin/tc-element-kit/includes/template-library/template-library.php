...
class ElementKit_Template_Library extends ElementKit_Template_Library_Base{
    ...
    function admin_menu() {
        ...
        if ( ! defined( 'TCEK_LO' ) ) {
            add_submenu_page(
                self::PAGE_ID,
                TCEK_TITLE,
                esc_html__( 'Template Library', 'tc-element-kit' ),
                'manage_options',
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script( 'ep-elementor-demo-importer-scripts', plugin_dir_url( __FILE__ ) . 'assets/js/element-kit-template-library.js', array( 'jquery' ), TCEK_VER, false );
            ...
            if ( ! is_array( $response_data ) || ! is_array( $sourceData ) ) {
                echo json_encode(
                    array(
                        'success' => false,
                        'id'      => '',
                        'edittxt' => esc_html__( 'Fail to upload. Try again.', 'tc-element-kit' )
                    )
                );
                wp_die();
            }
                ...
                if ( $importType == 'page' ) {
                    ...
                    if ( isset( $metaData['_elementor_data'] ) && isset( $metaData['_elementor_data'][0]  ) ) {
                        ...
                        echo json_encode(
                            array(
                                'success' => true,
                                'id'      => $new_post_id,
                                'edittxt' => ( $importType == 'page' ) ? esc_html__( 'Edit Page', 'tc-element-kit' ) : esc_html__( 'Edit Template', 'tc-element-kit' )
                            )
                        );
                        wp_die();
                    }
                    ...
                    echo json_encode(
                        array(
                            'success' => true,
                            'id'      => $template_id,
                            'edittxt' => esc_html__( 'Edit Template', 'tc-element-kit' )
                        )
                    );
                    wp_die();
                }
    ...
    public function admin_notice(){
        ...
        Notices::add_notice(
            [
                ...
                'message'          => __( 'This template library will be deprecated soon so please use our brand new template library in your editor (Make sure you activated it from <a href="admin.php?page=element_kit_options#element_kit_other_settings">element kit settings</a>).', 'tc-element-kit' ),
            ]
        );
    }
                    ...
                    <div class="tc-template-library-container tc-grid" tc-grid >
                        ...
                        <div class="tc-template-grid-container tc-width-3-4@m tc-width-4-5@l">
                            <div class="tc-flex tc-grid tc-margin-medium-bottom" tc-grid>
                                <div class="tc-grid-small tc-grid-divider tc-width-auto" tc-grid>
                                    <div>
                                        <ul class="tc-subnav tc-subnav-pill" tc-margin>
                                            <li class="pro-free-nagivation-item tc-active" data-filter="*"><a href="javascript:void(0)">All</a></li>
                                            ...
                                            <li class="template-category-item tc-hidden" data-demo="demo_search_result" id="demo_search_tab"><a href="javascript:void(0)"></a></li>
                                        </ul>
                                    </div>
                                </div>
